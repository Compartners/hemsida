...
import heroPoster from "@/assets/forest-drone.mp4";
...
const Hero = () => {
  ...
  const getOpenStatus = () => {
    ...
    if (isOpen) {
      return { isOpen: true, message: "Vi har öppet" };
    }

    if (day === 0 || day === 6) {
      return { isOpen: false, message: "Vi har stängt, öppnar åter måndag 08:00" };
    } else if (day === 5 && currentTime >= closeTime) {
      return { isOpen: false, message: "Vi har stängt, öppnar åter måndag 08:00" };
...
<div className="absolute inset-0 bg-gradient-to-t from-background/100 to-background/55" />      </div>

      {/* Background decorations */}
      <div className="absolute top-40 right-0 w-[500px] h-[500px] bg-primary/5 rounded-full blur-3xl z-0" />
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            <span className="relative flex h-3 w-3">
              {status.isOpen && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-500 opacity-75" />}
          ...
          <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6 leading-tight animate-fade-in" style={{ animationDelay: '0.1s' }}>
            Vi skräddarsyr er{" "}
            <span className="text-blue-300">telefonilösning</span>
            {" "}efter behov
          </h1>

          {/* Subheading */}
          <p className="text-lg md:text-xl text-white max-w-2xl mx-auto mb-8 animate-fade-in font-medium" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in" style={{ animationDelay: '0.3s' }}>
            <Button className="bg-secondary text-white hover:bg-secondary" size="xl" asChild>
              <a href="#kontakt">
                Kontakta oss
          ...
          {/* Tagline */}
          <p className="mt-16 text-2xl md:text-3xl font-display font-bold text-white animate-fade-in" style={{ animationDelay: '0.4s' }}>
            #comsåblirvipartners
          </p>
        </div>
      </div>
